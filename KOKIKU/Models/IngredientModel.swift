...
class IngredientModel {
    ...
    init(id: String, name: String, date: Date, storage: String) {
        self.id = id
        self.name = name
        self.date = date
    ...
    func notificationThreshold() -> Int {
        ...
        case "refrigerator", "Kulkas": // Adding Indonesian term
            return 5  // Show notification after 5 days in refrigerator
        case "room", "Rak Dapur": // Adding Indonesian term
    ...
    func notificationMessage() -> String {
        return "Bahan \(name) sudah \(daysSinceStored()) hari di \(storage), ketuk untuk mencari resep terbaik"
    }
